...
public class DefaultDataBase implements IDataBase {
    ...
    public RowSet queryProcedure(SqlCommand cmd) throws SQLException {
        ...
        try {
            setStatementParam(cs, params);
            cs.execute();
            return setProcedureParam(cs, params);
    ...
    private void setStatementParam(CallableStatement cs, ArrayList<ProcedureParam> params) throws SQLException {
        for (int i = 0; i < params.size(); ++i) {
            ...
            if (p.getParamIoType() != ProcParamType.INPUT
                ...
                cs.registerOutParameter(i, this.getSqlType(p.getParamDataType()));
            }
            ...
            if (p.getParamIoType() == ProcParamType.INPUT
                ...
                if (value != null) {
                    ...
                    cs.setNull(i + 1, pType);
                }
            }
        }
    }

    private RowSet setProcedureParam(CallableStatement cs, ArrayList<ProcedureParam> params) throws SQLException {
        CachedRowSetImpl rowSet = new CachedRowSetImpl();
        for (int i = 0; i < params.size(); ++i) {
